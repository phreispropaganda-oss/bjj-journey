...
export async function setUserActive(userId: string, active: boolean) {
  ...
  await (ctx.supabase.from('profiles') as ReturnType<typeof ctx.supabase.from>)
    .update({ active } as never).eq('id', userId)
...
export async function softDeleteUser(userId: string) {
  ...
  // Soft delete: keep record, mark deleted_at and deactivate
  await (ctx.supabase.from('profiles') as ReturnType<typeof ctx.supabase.from>)
    .update({
      active: false,
...
export async function restoreUser(userId: string) {
  ...
  await (ctx.supabase.from('profiles') as ReturnType<typeof ctx.supabase.from>)
    .update({ active: true, deleted_at: null } as never).eq('id', userId)
