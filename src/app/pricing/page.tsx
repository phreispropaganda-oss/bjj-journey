  ...
  return (
    <div className="min-h-screen bg-[#F7F4F0] px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-2xl font-bold text-ink-primary mb-2">Escolha seu plano</h1>
        <p className="text-sm text-ink-secondary">Invista na sua jornada no tatame</p>
        {!STRIPE_ENABLED && (
        ...
        <div className="flex items-center justify-center gap-3 mt-5">
          <span className={`text-sm font-medium ${!annual ? 'text-ink-primary' : 'text-ink-muted'}`}>Mensal</span>
          <button onClick={() => setAnnual(a => !a)}
            className={`w-12 h-6 rounded-full transition-colors relative ${annual ? 'bg-rise' : 'bg-[#DDD8D0]'}`}>
          ...
          <span className={`text-sm font-medium ${annual ? 'text-ink-primary' : 'text-ink-muted'}`}>
      ...
      <div className="space-y-4 max-w-sm mx-auto">
        {plans.map(plan => (
          <div key={plan.key} className={`rounded-2xl p-5 shadow-sm ${plan.highlight ? 'bg-rise text-white' : 'bg-brand-surface'}`}>
            {plan.highlight && (
            ...
            <div className="flex items-end justify-between mb-1">
              <h2 className={`text-xl font-bold ${plan.highlight ? 'text-white' : 'text-ink-primary'}`}>{plan.name}</h2>
              <div className="text-right">
                <span className="text-2xl font-bold">{formatPrice(annual ? plan.price.annual : plan.price.monthly)}</span>
                {plan.price.monthly > 0 && (
                  <span className={`text-xs ml-1 ${plan.highlight ? 'text-white/70' : 'text-ink-secondary'}`}>/{annual ? 'ano' : 'mês'}</span>
                )}
              </div>
            </div>
            <p className={`text-xs mb-4 ${plan.highlight ? 'text-white/80' : 'text-ink-secondary'}`}>{plan.description}</p>
            <ul className="space-y-2 mb-5">
              {plan.features.map(f => (
                <li key={f} className="flex items-center gap-2 text-sm">
                  <Check size={14} className={plan.highlight ? 'text-white' : 'text-green-500'} />
                  <span className={plan.highlight ? 'text-white' : 'text-ink-primary'}>{f}</span>
                </li>
              ))}
            </ul>
            <button
              ...
              className={`w-full py-3 rounded-full font-bold text-sm transition-colors disabled:opacity-60 ${
                plan.highlight ? 'bg-white text-rise hover:bg-white/90'
                : plan.key === 'free' ? 'border-2 border-[#DDD8D0] text-ink-secondary hover:border-[#FF6B2B] hover:text-rise'
                : 'bg-rise text-white hover:bg-[#E85A1A]'
              }`}>
