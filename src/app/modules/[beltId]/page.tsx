...
export default function ModulesPage({ params }: Props) {
  ...
  const { isCompleted, toggle, getCount } = useProgressStore()
  ...
  function handleComplete(bId: string, modId: string, techName: string, done: boolean) {
          ...
          return (
            <div key={mod.id} className="bg-white rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="h-[3px] bg-[#F0EDE8]">
                <div className="h-full rounded-none transition-all" style={{ width: `${pct}%`, background: mod.color }} />
              </div>
              {isOpen && (
                <div>
                  {mod.categories.map(cat => {
                    const catOpen = openCats[cat.id]
                    const catDone = cat.techniques.filter(t => isCompleted(`${beltId}-${mod.id}-${t.name.replace(/\s/g, '_')}`)).length
                    return (
