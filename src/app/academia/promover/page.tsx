          ...
          {[
            ...
            <Link key={t.href} href={t.href}
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
                t.href === '/academia/promover' ? 'bg-rise text-white' : 'bg-brand-elev text-ink-secondary'
              }`}>
              {t.label}
            </Link>
          ))}
      ...
      <div className="px-4 py-4 space-y-4 max-w-2xl mx-auto pb-10">
        ...
        <div className="bg-[#1A1A1A] rounded-2xl border border-[#2A2A2A] overflow-hidden">
          <div className="px-4 py-3 border-b border-[#2A2A2A]">
            <p className="text-[11px] font-black uppercase tracking-wider text-ink-secondary">Selecionar aluno</p>
          </div>
          {loading ? (
            <p className="text-ink-secondary text-sm px-4 py-4">Carregando...</p>
          ) : students.map(s => (
            <div key={s.id}
              onClick={() => selectStudent(s)}
              className={`flex items-center gap-3 px-4 py-3 border-b border-[#1E1E1E] cursor-pointer transition-all ${
                selected?.id === s.id ? 'bg-rise/10 border-rise/30' : 'hover:bg-brand-elev'
              }`}>
              ...
              {selected?.id === s.id && <span className="text-rise font-black">✓</span>}
            </div>
          ))}
        </div>
        ...
        {/* Promotion form */}
        {selected && (
          <div className="bg-[#1A1A1A] rounded-2xl p-4 border border-rise/30">
            <p className="text-[11px] font-black uppercase tracking-wider text-ink-secondary mb-4">
              Promovendo: <span className="text-white">{selected.name}</span>
            </p>

            {/* Belt selector */}
            <p className="text-[10px] font-bold uppercase tracking-wider text-ink-secondary mb-2">Nova faixa</p>
            <div className="space-y-2 mb-4">
              {BELTS.map(b => (
                <div key={b.id} onClick={() => { setToBelt(b.id); setToDegrees(0) }}
                  className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${
                    toBelt === b.id ? 'border-rise bg-rise/10' : 'border-brand-elev bg-brand-elev'
                  }`}>
                  ...
                  {toBelt === b.id && <span className="text-rise font-black">✓</span>}
                </div>
              ))}
            </div>
            ...
            {toBeltObj && (
              <>
                <p className="text-[10px] font-bold uppercase tracking-wider text-ink-secondary mb-2">Grau</p>
                  ...
                  {Array.from({ length: toBeltObj.maxDeg + 1 }, (_, i) => (
                    <button key={i} onClick={() => setToDegrees(i)}
                      className={`w-10 h-10 rounded-full border-2 font-black text-sm transition-all ${
                        toDegrees === i ? 'border-rise bg-rise text-white' : 'border-brand-elev text-ink-secondary'
                      }`}>{i}</button>
                  ))}
                </div>
              </>
            ...
            {/* Notes */}
            <p className="text-[10px] font-bold uppercase tracking-wider text-ink-secondary mb-2">Observação <span className="normal-case font-normal text-ink-muted">(opcional)</span></p>
            <textarea
              className="w-full bg-brand-elev border border-brand-elev rounded-xl px-3 py-2 text-white text-sm outline-none placeholder:text-ink-muted focus:border-rise resize-none mb-4"
              rows={2}
        ...
        {history.length > 0 && (
          <div className="bg-[#1A1A1A] rounded-2xl border border-[#2A2A2A] overflow-hidden">
            <div className="px-4 py-3 border-b border-[#2A2A2A]">
              <p className="text-[11px] font-black uppercase tracking-wider text-ink-secondary">Histórico de promoções</p>
            </div>
            {history.map((h, i) => (
