  ...
  return (
    <>
      <button onClick={() => { timer.start(minutes); setOpen(true) }}
        className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-full border border-[#DDD8D0] bg-white text-[#666] hover:bg-[#F0EDE8] transition-colors">
        <Timer size={12} />
        {minutes}min
      </button>
      {open && (
    ...
    <>
      ...
      <div className={`border-t border-[#E8E3DC] ${isDone ? 'opacity-70' : ''}`}>
        <div className="flex items-center gap-2.5 px-3.5 py-3 cursor-pointer active:bg-[#F7F4F0]" onClick={() => setOpen(o => !o)}>
          <span className="text-xs text-[#AAA] min-w-[18px] tabular-nums">{index + 1}</span>
          <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: moduleColor }} />
          <span className="flex-1 text-sm font-medium text-[#1A1A2E] leading-snug">{technique.name}</span>
          <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium flex-shrink-0 ${isDone ? 'bg-green-100 text-green-700' : 'bg-[#F0EDE8] text-[#AAA]'}`}>
            {isDone ? '✓ feito' : 'pendente'}
          </span>
          <ChevronDown size={14} className={`text-[#AAA] transition-transform flex-shrink-0 ${open ? 'rotate-180' : ''}`} />
        </div>
        {open && (
          <div className="px-3.5 pb-4 pt-0 bg-[#F7F4F0] border-t border-[#E8E3DC]">
            <div className="bg-[#F0EDE8] rounded-lg px-3 py-2.5 my-2.5 text-xs text-[#555] leading-relaxed border-l-2" style={{ borderColor: moduleColor }}>
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#AAA] block mb-1">Posição de entrada</span>
              {technique.entryPosition}
            </div>
            <p className="text-xs text-[#666] leading-relaxed mb-2.5">{technique.description}</p>
            <div className="space-y-1.5 mb-2.5">
              {technique.steps.map((step, i) => (
                <div key={i} className="flex gap-2.5 text-xs text-[#1A1A2E] leading-relaxed">
            ...
            <div className="flex gap-2 items-start bg-red-50 border border-red-200 rounded-lg p-2.5 mb-2 text-xs text-[#666] leading-relaxed">
              <AlertCircle size={14} className="text-red-500 flex-shrink-0 mt-0.5" />
              <span>{technique.commonMistake}</span>
            </div>
            <div className="flex gap-2 items-start bg-amber-50 border border-amber-200 rounded-lg p-2.5 mb-3 text-xs text-[#666] leading-relaxed">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <a href={`https://www.google.com/search?q=${encodeURIComponent(technique.name + ' jiu jitsu')}&tbm=isch`}
                target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-1 text-xs px-2.5 py-1.5 rounded-full border border-[#DDD8D0] bg-white text-[#666] hover:bs-[#F0EDE8] transition-colors">
              ...
              <button onClick={handleComplete}
                className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border font-semibold transition-colors ml-auto ${
                  isDone ? 'bg-green-100 border-green-300 text-green-700' : 'bg-white border-[#DDD8D0] text-[#666] hover:border-[#FF6B2B] hover:text-[#FF6B2B]'
                }`}>
